...
class _NormBase(Module):
    ...
    def __init__(
        ...
        # self.reset_parameters()
        if self.track_running_stats:
            self.register_buffer('running_mean_source', torch.zeros(num_features, **factory_kwargs))
            self.register_buffer('running_var_source', torch.ones(num_features, **factory_kwargs))
            ...
            self.running_mean_source: Optional[Tensor]
            self.running_var_source: Optional[Tensor]
            ...
            self.register_buffer("running_mean_source", None)
            self.register_buffer("running_var_source", None)
    ...
    def reset_running_stats(self) -> None:
        if self.track_running_stats:
            ...
            self.running_mean_source.zero_()  # type: ignore[union-attr]
            self.running_var_source.fill_(1)  # type: ignore[union-attr]
...
class _BatchNorm(_NormBase):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            bn_training = (self.running_mean_source is None) and (self.running_var_source is None)
        ...
        if self.training:
            ...
            z_source = F.batch_norm(
                ...
                self.running_var_source if not self.training or self.track_running_stats else None,
                self.weight,
                self.bias,
                bn_training,
