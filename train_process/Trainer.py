...
class Trainer(object):
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(
                ...
                with torch.no_grad():
                    predictions, boundary, feature = self.model_gen(data)
    ...
    def train_epoch(self):
        ...
        for batch_idx, sampleS in tqdm.tqdm(
            ...
            # oT, boundaryT, featureT = self.model_gen(imageT)

            imageS = sampleS['image'].cuda()
            target_map = sampleS['map'].cuda()
            target_boundary = sampleS['boundary'].cuda()
            # oS, boundaryS, featureS = self.model_gen(imageS)
            images_all = torch.cat((imageS, imageT), 0)
            o_all, boundary_all, feature_all = self.model_gen(images_all)
