...
from train_process import Trainer, Trainer_baseline, Trainer_beal, Trainer_posal, Trainer_baseline_wob \
    , Trainer_prototype, Trainer_prototype_full,Trainer_prototype_delete_en
...
def main():
    ...
    parser.add_argument(
        '--pro_weight', type=float, default=0.1, help='momentum',
    )
    parser.add_argument(
        '--global_pro_weight', type=float, default=0.9, help='momentum',
    )
    ...
    args.out = osp.join(here, 'logs', args.datasetT, args.method,
    'get_src_wobu_EA_use_PID_{}_use_TN_{}_use_fix_initial_{}_use_retrify_{}_gpw_{}_pw{}'.format(args.use_pid,
    ...
    ######################################################################
    if args.method == 'prototype_full' or args.method == 'prototype_delete_en':
    ...
    if args.method == 'baseline':
        ...
        trainer = Trainer_prototype.Trainer(
            cuda=cuda,
            model_gen=model_gen,
            model_dis=model_dis,
            model_uncertainty_dis=model_dis2,
    ...
    elif args.method == 'prototype_delete_en':
        trainer = Trainer_prototype_delete_en.Trainer(
            cuda=cuda,
            model_gen=model_gen,
            model_geninitial_pesudolabel=model_geninitial_pesudolabel,
            model_dis=model_dis,
